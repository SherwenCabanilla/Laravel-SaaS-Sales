        ...
        @forelse($funnels as $funnel)
            <label style="display:flex;align-items:center;gap:8px;">
                <input type="checkbox" name="funnel_ids[]" value="{{ $funnel->id }}"
                    {{ in_array($funnel->id, old('funnel_ids', $selectedFunnelIds ?? [])) ? 'checked' : '' }}>
                <span>{{ $funnel->name }}</span>
            </label>
        @empty
            <span style="color:#64748b;">No funnels yet. Leave blank and the coupon will work for all future funnels in this tenant.</span>
        @endforelse
    </div>
    <p style="margin-top:6px;color:#475569;font-size:12px;">Leave all unchecked to make the coupon valid in all tenant funnels.</p>
    @error('funnel_ids')<span style="color:red;font-size:12px;">{{ $message }}</span>@enderror
    @error('funnel_ids.*')<span style="color:red;font-size:12px;">{{ $message }}</span>@enderror
</div>
